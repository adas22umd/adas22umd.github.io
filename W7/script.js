...
function createMyChart() {
    ...
    const cuisineTypes = restaurants.map(function(restaurant){
        return restaurant.cuisine || "not defined"
    })
        ...
        myChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'A chart that talks about restaurant cuisines'
                    }
                },
                scales: {
                    y: {
...
function createMyMap() {
    ...
    try {
        ...
        const restaurantslocations = restaurants.filter(function(restaurant){
            return restaurant.neighborhood && restaurant.neighborhood !== 'unknown';
        })
        restaurantslocations.forEach(function(restaurant, index) {
            ...
            <div>
                ...
                <div>The cuisine is ${restaurant.cuisine}<br></div>
                <div>We rate the restaurant ${restaurant.rating}*<br></div>
                <em>${restaurant.neighborhood}</em>
            </div>`;
